...
# MobileNetV2 multiplier setting
parser.add_argument('--mb2_width_mult', default=1.0, type=float,
                    help='Width Multiplier for MobilenetV2(0~1)')
...
if __name__ == '__main__':
    ...
    for dataset_path in args.datasets:
        if args.dataset_type == 'voc':
            ...
            dataset = OpenImagesDataset(dataset_path, transform=train_transform, target_transform=target_transform,
    ...
    for epoch in range(last_epoch + 1, args.num_epochs):
        ...
        # save loss log
        with open(os.path.join(args.checkpoint_folder, 'loss_log.csv'), 'a') as f:
            writer = csv.writer(f)
            if epoch == 0:
                writer.writerow(['loss', 'location loss', 'classification loss'])
            writer.writerow([val_loss, val_regression_loss, val_classification_loss])
